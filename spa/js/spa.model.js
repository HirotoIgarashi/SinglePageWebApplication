...
spa.model = (function () {
  ...
  // イベントを提供する。peopleオブジェクトのパブリックメソッドには以下が
  // 含まれる。
  //  * get_user() - 現在のpersonオブジェクトを返す。
  //    現在のユーザがサインインしていない場合には、匿名personオブジェクト
  //    を返す。
  //  * get_db() - あらかじめソートされたすべてのpersonオブジェクト
  //    (現在のユーザを含む)のTaffyDBデータベースを含む返す。
  //  * get_by_cid( <client_id> ) - 指定された一意のIDを持つ
  //    personオブジェクトを返す。
  //  * login( <user_id> ) - 指定のユーザ名を持つユーザとしてログインする。
  //    現在のユーザオブジェクトは新しいIDを反映するように変更される。
  //    ログインに成功すると「spa-login」グローバルカスタムイベントを
  //    発行する。
  ...
  //    クライアントデータがバックエンドと同期していない場合のみ
  //    id属性と異なる。
  //  * id - 一意のID。オブジェクトがバックエンドと同期していない場合には
  //    未定義になることがある。
  //  * name - ユーザの文字列名。
  ...
  // chatオブジェクトはチャットメッセージングを管理するためのメソッドと
  // イベントを提供する。chatオブジェクトのパブリックメソッドには以下が
  // 含まれる。
  // * join() - チャットルームに参加する。このルーチンは、
  //   「spa-listchange」と「spa-updatechat」グローバルカスタムイベント
  //   のためのパブリッシャを含むバックエンドとのチャットプロトコルを
  //   確立する。
  //   現在のユーザが匿名の場合、join()は中断してfalseを返す。
  // * get_chatee() - ユーザがチャットしている相手のpersonオブジェクトを
  //   返す。
  //   チャット相手がいない場合はnullを返す。
  // * get_chatee( <person_id> ) - チャット相手をperson_idで特定される
  //   ユーザに設定する。person_idがユーザリストに存在しない場合は、
  //   チャット相手をnullに設定する。
  //   指定されたユーザがすでにチャット相手の場合はfasleを返す。
  //   「spa-setchatee」グローバルカスタムイベントを発行する。
  // * send_msg( <msg_text> ) - チャット相手にメッセージを発行する。
  ...
  //   アバター情報(personオブジェクトのcss_map)を含む「spa-listchange」
  //   イベントが発行される。
  //   update_avtr_mapは以下のような形式でなければいけない
  //   { person_id  : person_id, css_map  : css_map }
  //
  // このオブジェクトが発行するjQueryグローバルカスタムイベントには以下が
  // 含まれる。
  // * spa-setchatee - これは新しいチャット相手が設定されたときに
  //   発行される。
  ...
  //   または内容が変わったとき(ユーザのアバター詳細が変わったとき)に
  //   発行される。
  //   このイベントへの登録者は、更新データとしてpeopleモデルから
  //   people_dbを取得すべき。
  ...
  chat = (function () {
    ...
    _update_list = function ( arg_list ) {
      ...
      // チャット相手がオンラインでなくなっている場合は、チャット相手を
      // 解除する。
